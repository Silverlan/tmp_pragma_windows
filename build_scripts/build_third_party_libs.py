...
if platform == "win32":
	...
	env = os.environ.copy()
	env['CXXFLAGS'] = env.get('CXXFLAGS', '') + ' -Wno-error=array-bounds -Wno-array-bounds'
	env['CFLAGS']  = env.get('CFLAGS', '')  + ' -Wno-error=array-bounds -Wno-array-bounds'
	subprocess.run(["make", "-j", "-f", "../../cmpl_gcc.mak"], check=True, env=env)
